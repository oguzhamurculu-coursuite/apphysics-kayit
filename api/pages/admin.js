...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-top { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; flex-wrap: wrap; margin-bottom: 10px; }
  .card-info { flex: 1; }
...
<div class="modal-bg" id="modal-bg">
  <div class="modal">
    ...
    <div class="field"><label>Konular (her satıra bir konu)</label>
      <textarea id="f-topics" placeholder="Infinite Sequences and Series\nTaylor and Maclaurin"></textarea>
    </div>
...
function renderCourses() {
  ...
  grid.innerHTML = courses.map(c => {
    const fullBtn = c.full
      ? '<button class="btn btn-sm btn-success" onclick="toggleFull(\\''+c.id+'\\', false)">Doluluk Kaldır</button>'
      : '<button class="btn btn-sm btn-danger" onclick="toggleFull(\\''+c.id+'\\', true)">Dolu Yap</button>';
    const activeBtn = c.active
      ? '<button class="btn btn-sm" style="background:#888;color:#fff" onclick="toggleActive(\\''+c.id+'\\', false)">Pasif Yap</button>'
      : '<button class="btn btn-sm btn-success" onclick="toggleActive(\\''+c.id+'\\', true)">Aktif Yap</button>';
    return '<div class="course-card '+(c.full?'full':!c.active?'inactive':'')+'">'
      +'<div class="card-top"><div class="card-info">'
      +'<div class="card-tags">'
      +'<span class="tag tag-'+c.subject+'">'+(c.subject==='physics'?'AP Physics 1':'AP Calculus BC')+'</span>'
      +(c.online?'<span class="tag" style="background:#0f6e56;color:#e1f5ee">Online</span>':'')
      +(c.active&&!c.full?'<span class="tag tag-active">Aktif</span>':'')
      +(c.full?'<span class="tag tag-full">Dolu</span>':'')
      +(!c.active?'<span class="tag tag-inactive">Pasif</span>':'')
      +'</div>'
      +'<div class="card-title">'+c.title+'</div>'
      +'<div class="card-date">'+c.date+' · '+c.time+(c.duration?' · '+c.duration:'')+'</div>'
      +'</div></div>'
      +'<div class="card-actions">'
      +'<button class="btn btn-sm" style="background:#f0f0f0;color:#333" onclick="editCourse(\\''+c.id+'\\')">Düzenle</button>'
      +fullBtn+activeBtn
      +'<button class="btn btn-sm btn-danger" onclick="deleteCourse(\\''+c.id+'\\')">Sil</button>'
      +'</div>'
      +'<div class="topics-list">'+c.topics.map(t=>'<span class="topic">'+t+'</span>').join('')+'</div>'
      +'</div>';
  }).join('');
}
...
function openAddModal() {
  editingId = null;
  document.getElementById('modal-title').textContent = 'Yeni Ders';
  document.getElementById('f-subject').value = 'calculus';
  document.getElementById('f-title').value = '';
  document.getElementById('f-date').value = '';
  document.getElementById('f-time').value = '';
  document.getElementById('f-duration').value = '2h';
  document.getElementById('f-topics').value = '';
  document.getElementById('f-online').checked = true;
...
function editCourse(id) {
  ...
  if (!c) return;
  editingId = id;
  document.getElementById('modal-title').textContent = 'Dersi Düzenle';
  ...
  document.getElementById('f-topics').value = c.topics.join('\n');
  document.getElementById('f-online').checked = c.online;
  document.getElementById('f-active').checked = c.active;
  document.getElementById('f-full').checked = c.full;
  document.getElementById('modal-bg').classList.add('open');
}
...
async function saveModal() {
  const course = {
    ...
    topics: document.getElementById('f-topics').value.split('\n').map(t => t.trim()).filter(Boolean),
    online: document.getElementById('f-online').checked,
    active: document.getElementById('f-active').checked,
    full: document.getElementById('f-full').checked
  };
  if (editingId) { const idx = courses.findIndex(x => x.id === editingId); courses[idx] = course; }
...
async function toggleFull(id, full) { courses = courses.map(c => c.id===id?{...c,full}:c); await saveCourses(); renderCourses(); }
async function toggleActive(id, active) { courses = courses.map(c => c.id===id?{...c,active}:c); await saveCourses(); renderCourses(); }
